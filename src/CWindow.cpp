...
#define hAppInst App_GetInstance()
...
{
	uint8_t keyr = (cKeyColor >> 16) & 0xFF;
	uint8_t keyg = (cKeyColor >> 8) & 0xFF;
	uint8_t keyb = cKeyColor & 0xFF;
	uint8_t nkeyr = (cNewKeyColor >> 16) & 0xFF;
	uint8_t nkeyg = (cNewKeyColor >> 8) & 0xFF;
	uint8_t nkeyb = cNewKeyColor & 0xFF;
	...
	bmpdata = new uint8_t[bmp.bmHeight * bmp.bmWidth * 3];
	...
	for (int y = 0; y < bmp.bmHeight; y++)
	{
		...
		{
			int idx = y2 + x*3;
			uint8_t r = bmpdata[idx + 0];
			uint8_t g = bmpdata[idx + 1];
			uint8_t b = bmpdata[idx + 2];
...
bool WINCLASS::addMenuItem(uint32_t item, bool byPos, LPCMENUITEMINFO info)
{
